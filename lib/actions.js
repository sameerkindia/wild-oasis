...
// import { connectToDatabase } from "./database"; // Path to your connectToDatabase.js
...
export async function getAllCabin() {
  ...
  try {
    ...
    // console.log(cabins , " This was Cabins")
    return cabins;
  } catch (error) {
...
export async function getOneCabin(id) {
  try {
    ...
    // console.log(cabin , ' this was single cabin')
    return cabin;
  } catch (error) {
...
export async function getBookedDatesByCabinIdServer(cabinId) {
  // console.log(cabinId , "this is cabin id")
  // console.log(new ObjectId(cabinId) , "this is cabin object id")
  try {
    const client = await connectToDatabase();
    const db = client.db("wild-oasis");
...
export async function createBooking(bookingData, formData) {
  try {
    ...
    revalidatePath(`/cabins/${bookingData.cabinId}`); // Replace with your path revalidation method
    redirect("/cabins/thankyou"); // Replace with your redirect method
  } catch (error) {
...
export async function getGuestBookings(guestId) {
  try {
    ...
    const bookings = await db.collection("bookings").find({ guestId: guestId}).toArray();
    if (!bookings.length) return [];

    const enrichedBookings = await Promise.all(
      bookings.map(async (booking) => {
        const cabin = await db.collection("cabins").findOne({ _id: booking.cabinId});
