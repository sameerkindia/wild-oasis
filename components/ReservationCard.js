...
async function ReservationCard({ booking, onDelete }) {
  ...
  return (
    <div className="flex border border-primary-800 max-sm:flex-col">
      <div className="relative h-32 aspect-square">
        <Image src={`data:image/png;base64,${imageData}`} fill alt={`Cabin ${name}`} className="border-r border-primary-800 object-cover" />
      </div>

      <div className="flex-grow px-6 py-3 flex flex-col gap-2 2md:gap-0">
        ...
        <div className="flex gap-5 mt-auto items-baseline max-sm:flex-wrap">
